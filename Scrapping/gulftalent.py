...
class GulfTalent(WebScraper):
    ...
    def start(self):
        try:
            #print('GULFTALENT STARTED')
            self.extractor()
            self.exportToDB()
            #print('GULFTALENT FINISHED')
        except:
            print("ERROR")
    ...
    def extractor(self):
        ...
        for job in self.response_api["results"]["data"]:
            matching = self.get_match_rate(self.extractedJobs.get("FullJobDescription")["description"])
            if matching == 0:
                continue
